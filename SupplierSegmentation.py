...
def perform_clustering():
    ...
    features = ['product_count', 'total_quantity_sold', 'avg_sale_price', 'unique_customers']
...
def plot_results(df):
    ...
    plt.subplot(1, 2, 1)
    sns.scatterplot(data=df, x='product_count', y='total_quantity_sold', hue='cluster', palette='viridis')
    plt.title('Ürün Sayısı vs Toplam Satış Miktarı')
    
    plt.subplot(1, 2, 2)
    sns.scatterplot(data=df, x='avg_sale_price', y='unique_customers', hue='cluster', palette='viridis')
    plt.title('Ortalama Fiyat vs Benzersiz Müşteriler')
...
@app.get("/")
async def root():
    return {"message": "Tedarikçi Segmentasyonu API'sine Hoş Geldiniz"}
